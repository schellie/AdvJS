...
//COMMON /PLACOM/ HOLDNG
//COMMON /MTXCOM/ MTEXT
//COMMON /WIZCOM/ WKDAY,WKEND,HOLID,HBEGIN,HEND,HNAME,SHORT,MAGIC,MAGNM,LATNCY,SAVED,SAVET,SETUP
//
...
//	HERE(OBJ)  = PLACE[OBJ] == LOC || TOTING(OBJ) 
//	AT(OBJ)  = PLACE[OBJ] == LOC || FIXED[OBJ] == LOC
//	LIQ2(PBOTL)  = (1-PBOTL) *WATER+(PBOTL/2) *(WATER+OIL) 
//	LIQ(DUMMY)  = LIQ2(MAX0(PROP[BOTTLE],-1-PROP[BOTTLE]) ) 
//	LIQLOC(LOC)  = LIQ2(((COND[LOC]/2*2%8) -5) *(COND[LOC]/4%2) +1) 
//	BITSET(L,N)  = (COND[L] && SHIFT(1,N) )  ! =  0
//	FORCED(LOC)  = COND[LOC] == 2
//	DARK(DUMMY)  = (COND[LOC]%2)  == 0 && (PROP[LAMP] == 0 || 
//	1	 !HERE(LAMP) ) 
//	PCT(N)  = RAN(100)  < N
...
//	J = 0
		for (var I = 1; I <= 4; I++) { 
			if (A[I] != BLANKS) J = 1;
//9
			A[I] = A[I] && (SHIFT((A[I]  && '@@@@@') ,-1) ^ -1);  }
		if (BLKLIN && J == 0) {}//GOTO 2;
//
//	SECOND = 0
//	WORD1 = A[1] 
//	WORD1X = A[2] 
...
//	if (((A[J] .XOR.BLANKS)  && MSK)  == 0) GOTO 15
//	if (SECOND == 0) GOTO 10
//	MSK = -MASKS[6-K] 
//	WORD2 = (SHIFT(A[J] ,7*(K-1) )  && MSK) 
//	1	+(SHIFT(A[J+1] ,7*(K-6) )  && (-2-MSK) ) 
//	WORD2X = (SHIFT(A[J+1] ,7*(K-1) )  && MSK) 
//	1	+(SHIFT(A[J+2] ,7*(K-6) )  && (-2-MSK) ) 
//	RETURN
//
//15	if (SECOND == 1) GOTO 10
//	SECOND = 1
//	if (J == 1) WORD1 = (WORD1 && -MASKS[K] ) 
//	1	 || (BLANKS && (-MASKS[K] .XOR.-1) ) 
//10
				};
			};
//	RETURN
...
//	if (REPLY == 'YES' || REPLY == 'Y') GOTO 10
//	if (REPLY == 'NO' || REPLY == 'N') GOTO 20
//	TYPE 9
...
//2	if (FROM > 0 && FROM <=  300) CALL CARRY(OBJECT,FROM) 
...
//	CALL DATIME(D,T) 
//	PRIMTM = WKDAY
//	if ( D % 7  <=  1) PRIMTM = WKEND;
//	if (D >=  HBEGIN && D <=  HEND) PRIMTM = HOLID
//	PTIME = (PRIMTM && SHIFT(1,T/60) )  ! =  0
//	SOON =  false
//	if (SETUP >=  0) GOTO 20
...
//	D = MAGNM
		/*for (Y = 1; Y <= 5, Y++) { 
			Z = (Y%5) +1
			X = (Math.abs(VAL[Y] -VAL[Z] ) *(D%10) + (T%10) % 26) +1
			T = T/10
			D = D/10
...
//
//10	FROM = FROM+1
//	if ((H && SHIFT(1,FROM) )  ! =  0) GOTO 10
//	if (FROM >=  24) GOTO 20
//	TILL = FROM
//14	TILL = TILL+1
//	if ((H && SHIFT(1,TILL) )  == 0 && TILL ! =  24) GOTO 14
//	if (FIRST) TYPE 16,DAY1,DAY2,FROM,TILL
//	if ( !FIRST) TYPE 18,FROM,TILL
...
/*		for (I = FROM; I <= TILL, I++) {
			IDONDX = I
5			NEWHRX = (NEWHRX || SHIFT(1,IDONDX) ) }*/
//	GOTO 10
//	END
//
//
...
//11			SHIFT = ((SHIFT && "377777777777) /2) +J 
		//	}
//20	RETURN
/*30		for (I = 1; I <= DIST, I++) {
			J = 0*/
//	if ((SHIFT && "200000000000)  ! =  0) J = "400000000000
//31			SHIFT = (SHIFT && "177777777777) *2+J 
		//	}
//	RETURN
//	END
//
